...
export default function App() {
  ...
  // --- CORE STATE DRIVERS ---
  // Premium entry experience: controls the custom animated splash screen presentation
  ...
  useEffect(() => {
    ...
    if (stored) {
      try {
        ...
        // Let's elegantly reconcile the seed invoices so any updates to names/text in data.ts 
        // propagate to the app instantly, while preserving all other custom user-created invoices.
        let changed = false;
        const reconciled = parsed.map((inv) => {
          const freshSeed = INITIAL_INVOICES.find((seed) => seed.id === inv.id);
          if (freshSeed) {
            // Check if any fields changed so we only trigger writes if necessary
            if (JSON.stringify(inv) !== JSON.stringify(freshSeed)) {
              changed = true;
              return freshSeed;
            }
          }
        ...
        if (changed) {
          localStorage.setItem(LOCALSTORAGE_KEY, JSON.stringify(reconciled));
        }
      } catch (err) {
  ...
  return (
    <>
      ...
      <footer className="no-print bg-slate-100 border-t border-slate-200 py-4 text-center text-xs text-slate-500 flex flex-col sm:flex-row items-center justify-between px-6 sm:px-8 max-w-7xl w-full mx-auto gap-4">
        <div>
          <span className="font-serif tracking-wider font-semibold">DONS LEATHER ATELIER</span> — Professional Invoice Registrar
        </div>
        <div className="flex items-center gap-4">
          <button 
            type="button"
            onClick={() => {
              if (window.confirm("Are you sure you want to reset all invoices to the default seeds from the code? This will replace your current browser session database and show your updated customer names.")) {
                localStorage.removeItem(LOCALSTORAGE_KEY);
                window.location.reload();
              }
            }}
            className="text-brand-wine hover:text-brand-wine-light font-medium underline transition-all cursor-pointer bg-transparent border-none p-0"
          >
            Reset Ledger to Code Defaults
          </button>
        </div>
      </footer>
